...
export default function Home() {
  ...
  const send=(setMsgs,ctx)=>{
    if(!inp.trim())return;
    const reply=lunaReply(ctx);
    setMsgs(p=>[...p,{role:"u",text:inp}]);setInp("");setTyping(true);
    setTimeout(()=>{setTyping(false);setMsgs(p=>[...p,{role:"a",text:reply}]);},1200+Math.random()*800);
  };
